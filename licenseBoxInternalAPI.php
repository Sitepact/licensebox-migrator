...
class LicenseBoxInternalAPI{
	...
	public function __construct(){ 
		$this->api_url = 'ADD YOUR OWN';
		$this->api_key = 'ADD YOUR OWN';
	...
	public function search_licenses($keyword){
		$data_array =  array(
			"keyword"  => $keyword
		);
		$get_data = $this->call_api(
			'POST',
			$this->api_url.'api/search_license', 
			json_encode($data_array)
		);
		$response = json_decode($get_data, true);
		return $response;
	...
	public function deactivate_license_activations($license_code){
		$data_array =  array(
			"license_code"  => $license_code
		);
		$get_data = $this->call_api(
			'POST',
			$this->api_url.'api/deactivate_license_activations', 
			json_encode($data_array)
		);
		$response = json_decode($get_data, true);
		return $response;
